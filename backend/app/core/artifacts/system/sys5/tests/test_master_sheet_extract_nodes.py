...
def test_app_param_extract_handles_numeric_cells():
    ...
    # never escalates to the LLM - no stub needed for run_agent_with_structured_output.
    rows = [
        {
    ...
    node = app_param_extract.build(_FakeStore(rows), llm=None, tools=[], pipeline_config=PipelineConfig())
...
def test_comm_matrix_extract_handles_numeric_cells():
    ...
    node = comm_matrix_extract.build(_FakeStore(rows), llm=None, tools=[], pipeline_config=PipelineConfig())
...
def test_io_signal_extract_handles_numeric_cells():
    ...
    node = io_signal_extract.build(_FakeStore(rows), llm=None, tools=[], pipeline_config=PipelineConfig())
