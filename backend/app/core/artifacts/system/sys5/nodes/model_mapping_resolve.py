...
def build(store: InMemoryWorkbookStore, llm, tools: list):
    def node(state: PipelineState) -> PipelineState:
        ...
        for factor in [*table.fixed_factors, *table.variable_factors]:
            for value in factor.values:
                ...
                if factor.signal_ref:
                    ...
                    match = excel_io.fuzzy_find(value, [r.test_case_input or "" for r in rows], threshold=_FUZZY_INPUT_THRESHOLD)
        ...
        if unresolved:
            ...
            result, _ = run_agent_with_structured_output(llm, tools, prompt, user_input, _ResolutionBatch)
