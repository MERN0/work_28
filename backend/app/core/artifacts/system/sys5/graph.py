...
def _build_outer_graph(
    ...
    graph.add_node("comm_matrix_extract", comm_matrix_extract.build(store, pipeline_config))
    graph.add_node("app_param_extract", app_param_extract.build(store, pipeline_config))
    graph.add_node("io_signal_extract", io_signal_extract.build(store, pipeline_config))
    graph.add_node("test_pattern_gen", test_pattern_gen.build(store, llm, pipeline_config))
    graph.add_node("model_mapping_resolve", model_mapping_resolve.build(store, pipeline_config))
    graph.add_node("compound_command_map", compound_command_map.build(store, pipeline_config))
